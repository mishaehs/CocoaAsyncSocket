...
- (void)doReadData
{
	...
	if (!done && !error && !socketEOF && !waiting && hasBytesAvailable)
	{
		...
		if (bytesRead > 0)
		{
			// Check to see if the read operation is done
      if (currentRead->term != nil) {
        NSData * data = [NSData dataWithBytes:(const void *)[preBuffer readBuffer] length:bytesToRead];
        NSData *strData = [data subdataWithRange:NSMakeRange(0, [data length])];
        NSString *msg = [[NSString alloc] initWithData:strData encoding:NSUTF8StringEncoding];
        NSMutableArray *contents = (NSMutableArray *)[[msg componentsSeparatedByString:@"\x0A"] mutableCopy];
        ...
        for(NSString *line in contents) {
          if ([line isEqual:@""]) {
            ...
            headers[key] = [parts componentsJoinedByString:@":"];
          }
        }
        NSLog(@"%@",headers);
        for (NSString* key in headers.allKeys)
        {
          if ([key isEqualToString:@"content-length"]) {
            unsigned n;
            sscanf([headers[key] UTF8String], "%u", &n);
            currentRead->term = nil;
            currentRead->readLength = n;
            currentRead->buffer = [[NSMutableData alloc] initWithLength:n];
            currentRead->bufferOwner = YES;
            currentRead->originalBufferLength = 0;
          }
        }
      }
        ...
        NSLog(@"%lu->%lu", (unsigned long)currentRead->bytesDone, (unsigned long)currentRead->readLength);
				done = (currentRead->bytesDone == currentRead->readLength);
			}
			else if (currentRead->term != nil)
			{
